...
<%
/*
모든영역(페이지, 리퀘스트, 세션, 어플리케이션)에는 어떠한 타입의 객체도
저장이 가능하다. 즉 Object타입으로 자동 형변환되어 저장되고 사용시에는
각 객체로 형변환후 사용하면 된다. 차후 EL(표현식)을 사용하면 형변환없이
사용하는것이 가능하다. 

1.page영역
	- page영역에 저장된 속성은 해당 페이지에서만 공유되고, 페이지를 
	벗어나는 순간 소멸된다. 
	- page영역에는 pageContext 객체를 사용하여 영역을 사용한다. 
*/
...
<body>
	...
	읽어올때도 Object타입으로 읽어오는것이 기본이다. 
	*/
	Object obj = pageContext.getAttribute("pageDate");
	String dateString = "";
	//저장된 객체가 Date타입으로 형변환후 ...
	...
	//MemberDTO타입으로 형변환후 데이터를 꺼내어 출력
	...
	<%-- 
		페이지 영역에 저장된 값은 다른 페이지로 이동하면 속성값이 
	...
	<%-- 
		외부 파일을 인클루드 하는 경우 외부문서는 원본 그대로 현재 문서에
		포함된후 컴파일이 진행되므로, include 페이지 지시어를 사용하는경우
