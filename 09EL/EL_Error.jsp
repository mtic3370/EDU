...
<fieldset>
	...
	<!-- 배열이나 컬렉션처럼 인덱스로 접근하는 경우.(닷)은 사용할수 없다. -->
	<ul>
	...
	<!-- JSTL의 forEach 태그를 통해 반복출력. 해당문장은 배열이나 컬렉션의 크기만큼
	반복하는 확장for문 형태로 기술됨 -->
	<h4>배열</h4>
	...
	<ul>
		...
		<!-- EL을 통해 사용자정의 객체를 출력하는경우 각각의 속성은 getter()
		메소드를 통해 접근하게 되므로 getter()메소드가 정의되지 않으면 에러가 
		발생된다.  -->	
		<li>속성은 존재,  getter가 없는경우 : ${eldto.id }:
		\${eldto.id}[500에러발생]</li>
		<li>el변수명이 틀린경우 : \${noeldto.pwd}:${noeldto.pwd}:[에러no, 미출력]
		</li>
	</ul>
	...
	<ul>
		...
		<!-- 
		EL을 통해 4가지 영역에 접근할때 XXXscope패턴으로 사용함.
		pageContext는JSP에서 접근기 사용됨.
		 -->				
